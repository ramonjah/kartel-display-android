...
## Текущий статус — Этап 5 (Renderer MVP)

- ✅ Pairing (код с экрана → owner подтверждает в BUSINESS UI → токен single-read)
- ✅ Получение Layout/Playlist через `display_get_screen_config` — один раз при старте
- ✅ Рендер зон: `clock`, `image`, `qr` (остальные типы — `UnknownWidget`-плейсхолдер, честно, не притворяется)
- ✅ Heartbeat раз в 30с
- ❌ Realtime-подписка (diff по `layout_version`) — Этап 6
- ❌ Offline-кэш ассетов, локальный лог событий — Этап 6/7
...
| `storage/` | `TokenStore` — Keystore-backed (не plaintext) |
